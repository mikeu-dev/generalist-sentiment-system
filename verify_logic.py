...
def test_system():
    ...
    try:
        ...
        analyzer = SentimentAnalyzer(model_path='test_model_verify.pkl')
        
        # 2. Load Training Data
        print("[2/7] Loading Training Data...")
        if not os.path.exists('sample_training.csv'):
            print("❌ Error: sample_training.csv not found.")
            return
        df_train = pd.read_csv('sample_training.csv')
        ...
        if not os.path.exists('sample_reviews.csv'):
            print("❌ Error: sample_reviews.csv not found.")
            return
        df_test = pd.read_csv('sample_reviews.csv')
        ...
        # Cleanup
        if os.path.exists('test_model_verify.pkl'):
            os.remove('test_model_verify.pkl')
